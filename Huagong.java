...
public class HuaGong {
    ...
    Map<String, Double> chapter_2_2(){
        ...
        x1 = (t1-40)*(0.62-0.82)/(45-40)+0.82;
        y1 = (t1-40)*(0.83-0.93)/(45-40)+0.93;

        x2 = (t2-55)*(0.18-0.31)/(60-55)+0.31;
        y2 = (t2-55)*(0.38-0.57)/(60-55)+0.57;
    ...
    Map<String,Integer> chapter_3_2(double tbs,double llb){
    ...
    Map<String,Double> chapter_4_1(int Nj,int Nt){
    ...
    Map<String,Double> chapter_4_2(double t1,double t2,double x1,double y1,double x2,double y2,double PM1,double PM2){
        ...
        pA = (t1 - 40) * (583.7 - 605.5) / (60 -40) + 605.5;
        pB = (t1 - 40) * (620.0 - 638.9) / (60 -40) + 638.9;
        pA1 = (t2 - 40) * (583.7 - 605.5) / (60 -40) + 605.5;
        pB1 = (t2 - 40) * (620.0 - 638.9) / (60 -40) + 638.9;
    ...
    Map<String,Double> chapter_5_4(double PV1,double PV2,double D,double Wd,double Vs1,double Vs2){
        ...
        System.out.println("\n(3) 浮阀数n与开孔率>>>");
        System.out.println("\t精馏段：");
        double u0 = F0 / Math.pow(PV1,0.5);
        System.out.println("\tu0="+u0);
        double n = 4 * Vs1 / (u0 * 0.039 * 0.039 * 3.14);
        int n_after = (int)n + 1;
        System.out.println("\tn="+n_after+"块");
        double fie = ((int)n + 1) * Math.pow(0.039,2) / Math.pow(1.6,2);
        System.out.println("\tφ="+fie * 100+"%");
        System.out.println("\t提馏段：");
        double u01 = F0 / Math.pow(PV2,0.5);
        System.out.println("\tu0="+u01);
        double n1 = 4 * Vs2 / (u01 * 0.039 * 0.039 * 3.14);
        int n1_after = (int)n + 1;
        System.out.println("\tn="+n1_after+"块");
        double fie1 = ((int)n1 + 1) * Math.pow(0.039,2) / Math.pow(1.6,2);
        System.out.println("\tφ="+fie1 * 100+"%");
        ...
        HashMap<String, Double> map = new HashMap<>();
        //TODO 添加一些变量到map中
        map.put("N",n_after * 1.0);
        map.put("N1",n1_after * 1.0);
        return map;
    ...
    void chapter_6_1(double D,double Wd,double AT,double Af,double PV1,double PV2,double PL1,double PL2){
        ...
        double a1 =  Math.pow(PV2 / (PL2 - PV2),0.5);
    ...
    public static void main(String[] args) {
        ...
        System.out.println("年处理量："+huaGong.year_process_mount+"吨,进料组成："+ huaGong.mass_frac * 100 +"%");
        ...
        for (int i = 1; i < huaGong.Xn.length; i++) {
            if (huaGong.Xn[i] == 0.0){
                break;
            }
            System.out.print("\tY" + i +" = " + huaGong.Yn[i]);
            System.out.println("\tX"+ i +" = " + huaGong.Xn[i] +
                    (huaGong.Xn[i] < huaGong.Xf ? " <Xf " : "")
                    +
                    (huaGong.Xn[i] < huaGong.Xw ? " <Xw " : "")
            );
        }


        Map<String, Integer> map2 = huaGong.chapter_3_2(map1.get("tbs"), map1.get("llb"));
